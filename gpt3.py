...
# eval_iters = 200
# n_embd = 384 # Self-attention embedding dimension
...
#         model = BigramModel().to(device)
#         i = int(sys.argv[2])
#         model.load_state_dict(torch.load(f'models/model{i}.pt'))
#         train(model=model, i=i)
# else:
#     i = int(sys.argv[1])
#     model = BigramModel().to(device)
#     model.load_state_dict(torch.load(f'models/model{i}.pt'))
#     print(decode(model.generate(idx=torch.zeros((1, 1),
#                                                 dtype=torch.long, device=device),
#                                 max_new_tokens=5000)[0].tolist()))
